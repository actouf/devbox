...
namespace Disko\CoreBundle\Traits;
...
trait GaufretteTrait
{
    ...
    public function preUploads()
    {
        ...
        foreach($attributes as $attribute)
        {
            ...
            if (null !== $this->$file) {
                $this->removeOneUpload($attribute);
                $this->$path = $this->slugify($this->getClassName()).'/'.$this->slugify($attribute).'/'.uniqid().'_'.$this->slugify($this->$file->getClientOriginalName()).'.'.$this->$file->guessExtension();
            }
        }
    }
